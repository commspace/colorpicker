  ...
  <div onselectstart="return false">
    ...
    >
      ...
      <div class="activeMode-wrapper">
        <div
          class="btn-activeMode solid"
          :class="activeMode === 'solid' ? '' : 'gray'"
          @click.stop="changeMode('solid')"
        ></div>
        <div
          class="btn-activeMode linear"
          :class="activeMode === 'linear' ? '' : 'gray'"
          @click.stop="changeMode('linear')"
        ></div>
        <div
          class="btn-activeMode radial"
          :class="activeMode === 'radial' ? '' : 'gray'"
          @click.stop="changeMode('radial')"
        ></div>
        <slot name="after-active-mode"></slot>
      </div>
      <div
        class="grad-wrapper"
      ...
      >
        ...
        >
          <div
            ...
            @click.stop="getGradPickerPos($event.target, index)"
      ...
      <label class="hex-wrapper">
        <span class="hex-wrapper-label">Hex </span>
        <input
          type="text"
          class="hex-wrapper-input"
          v-model="hex"
          @input="hexEvent"
          maxlength="6"
        />
        <i class="hex-wrapper-icon"></i>
        <slot name="in-hex-wrapper"></slot>
      </label>
...
const hex = ref(Utils.rgb2hex(props.color));
...
watch(paletteColor, () => {
  updateGradColor();
  updatePreviews();
  hex.value = Utils.rgb2hex(Utils.hsb2rgb(paletteColor));
});
const updateCurrentPalletFromRGB = (color) => {
  updateCurrentPallet(Utils.rgba2hsba(color));
};
const updateCurrentPallet = (color) => {
  paletteColor.a = color.a;
  paletteColor.b = color.b;
  paletteColor.h = color.h;
  paletteColor.s = color.s;
};
